...
void main()
{
    ...
    float b2 = 3. * u * u * (1.-u);
    float b3 = u * u * u;
    vec4 curvePos = b0*p0 + b1*p1 + b2*p2+ b3*p3;
    ...
    float ua = radiusCircle*cos(2.0*pi*angle);
    float va = radiusCircle*sin(2.0*pi*angle);
    float ta = 0.f;
    ...
    vec4 pos =  curvePos + vec4(aux, 1.0f);

    //sem rotacao
    //vec4 pos =  curvePos + vec4(ua,va,ta, 1.0f);
    ...
    vNormal = normalize(vec3(curvePos - pos));
